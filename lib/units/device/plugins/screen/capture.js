  ...
  .define(function(options, adb, router, push, storage, minicap, display) {
    ...
    plugin.capture = function() {
      ...
      return minicap.run(util.format(
        ...
        .catch(function(err) {
          log.warn('Minicap capture failed, falling back to adb screencap: %s', err.message)
          // Try to clean up temp file just in case
          adb.shell(options.serial, ['rm', '-f', file]).catch(function() {})

          return adb.screencap(options.serial)
            .then(adbkit.util.readAll)
